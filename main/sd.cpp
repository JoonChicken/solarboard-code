...
void logging_task(void *arg_ptr) {
    ...
    while (true) {
        ...
        if (insert_idx <= remove_idxs[which] + args.write_size) {
            // yield 10 ms, to let idle and maybe integration run
            // only do this if we didnt have to write a lot
            // we should only have to write < write size if
            // 1. we are outpacing the writing task - in which case we can yield
            // 2. we got to the end of the buffer - if we get to the end without the writing task changing the buffer, we're faster and can yield
...
Expected<std::monostate> SDCard::create_file_numbered_name(const char* name, const char* ext,  const uint8_t* data, size_t length, char* filename, size_t buf_size) {
    bool broke = false;
    struct stat st;
    for (int i = 0; i < 1000; i++) {
        ...
        snprintf(filename, buf_size, "%s/%s%d.%s", MOUNT_POINT, name, i, ext);
        ESP_LOGI(TAG, "filename: %s, res: %d", filename, stat(filename, &st));
    ...
    if (!broke) {
        snprintf(filename, buf_size, MOUNT_POINT"/%s.%s", name, ext);
    }
    ESP_LOGI(TAG, "filename: %s", filename);

    return this->create_file(filename, data, length);
}
